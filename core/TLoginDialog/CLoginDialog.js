...
//        jq.registerEventHandler(this.name+".model","onfetch",[this,'_onFetchModel']);
    ...
    login:function(){
        ...
        var _login = form.login.value;
        if(!_login){alert('Введите логин!'); return;}
        var arg = { login:_login };
        jq.get('TActionServer').registerCommand('TAccountService','authenticatePhase1',arg,[this,'_onAtthPhase1']);
    },
    logout:function(){
    ...
    _onAtthPhase1:function(e){
      if(e.status>0) jq.event(this,'onerror',e);
         ...
         var form = this.id.getElementsByTagName('form')[0];  
         var _pass = form.pass.value;
         var _login = form.login.value;
         var _rm = form.rm.checked?1:0;
         var arg = {
           login:_login,
           pass:this.md5(this.md5(_pass)+e.pub_key),
           rm:_rm
         };
         jq.get('TActionServer').registerCommand('TAccountService','authenticatePhase2',arg,[this,'_onAtthPhase2']);
//         this.model.fetch({},jq.FETCH_IF_UPDATE);
      }
    },
